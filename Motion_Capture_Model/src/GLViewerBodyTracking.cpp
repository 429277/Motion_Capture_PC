#include "GLViewerBodyTracking.hpp"
#include <random>
#include "Shader.hpp"
...
void addVert(Simple3DObjectBodyTracking &obj, float i_f, float limit, float height, sl::float4 &clr) {
...
GLViewerBodyTracking* currentInstance_ = nullptr;
...
GLViewerBodyTracking::GLViewerBodyTracking() : available(false) {
...
GLViewerBodyTracking::~GLViewerBodyTracking() {
}

void GLViewerBodyTracking::exit() {
    if (currentInstance_) {
        available = false;
    }
}

bool GLViewerBodyTracking::isAvailable() {
    glutMainLoopEvent();
    return available;
}
...
void GLViewerBodyTracking::init(int argc, char **argv) {
    ...
    // Create the camera
    camera_ = CameraGLBodyTracking(sl::Translation(0, 0, 0), sl::Translation(0, 0, -100));
    //camera_.setOffsetFromPosition(sl::Translation(0, 0, 1000));

    // Create the skeletons objects
    skeletons = Simple3DObjectBodyTracking(sl::Translation(0, 0, 0), false);
    skeletons.setDrawingType(GL_QUADS);

    // Set background color (black)
    bckgrnd_clr = sl::float4(0.2f, 0.19f, 0.2f, 1.0f);

    floor_grid = Simple3DObjectBodyTracking(sl::Translation(0, 0, 0), false);
    ...
    glutDisplayFunc(GLViewerBodyTracking::drawCallback);
    glutMouseFunc(GLViewerBodyTracking::mouseButtonCallback);
    glutMotionFunc(GLViewerBodyTracking::mouseMotionCallback);
    glutReshapeFunc(GLViewerBodyTracking::reshapeCallback);
    glutKeyboardFunc(GLViewerBodyTracking::keyPressedCallback);
    glutKeyboardUpFunc(GLViewerBodyTracking::keyReleasedCallback);
    glutCloseFunc(CloseFunc);

    available = true;
}

void GLViewerBodyTracking::render() {
    if (available) {
...
template<typename T>
void createSKPrimitive(sl::BodyData& body, const std::vector<std::pair<T, T>> &map, Simple3DObjectBodyTracking &skp, sl::float4 clr_id) {
    for (auto& limb : map) {
...
void createSKPrimitivePtOnly(sl::BodyData& body, const int idx_start, const int idx_end, Simple3DObjectBodyTracking &skp, sl::float4 clr_id) {
...
void GLViewerBodyTracking::updateData(Bodies &bodies, sl::Transform& pose) {
    mtx.lock();
    skeletons.clear();
    cam_pose = pose;
...
void GLViewerBodyTracking::update() {
...
void GLViewerBodyTracking::draw() {
...
void GLViewerBodyTracking::clearInputs() {
...
void GLViewerBodyTracking::drawCallback() {
    currentInstance_->render();
}

void GLViewerBodyTracking::mouseButtonCallback(int button, int state, int x, int y) {
    if (button < 5) {
        if (button < 3) {
...
void GLViewerBodyTracking::mouseMotionCallback(int x, int y) {
...
void GLViewerBodyTracking::reshapeCallback(int width, int height) {
...
void GLViewerBodyTracking::keyPressedCallback(unsigned char c, int x, int y) {
    currentInstance_->keyStates_[c] = KEY_STATE::DOWN;
    //glutPostRedisplay();
}

void GLViewerBodyTracking::keyReleasedCallback(unsigned char c, int x, int y) {
    currentInstance_->keyStates_[c] = KEY_STATE::UP;
}

void GLViewerBodyTracking::idle() {
    glutPostRedisplay();
}

Simple3DObjectBodyTracking::Simple3DObjectBodyTracking() : vaoID_(0) {}

Simple3DObjectBodyTracking::~Simple3DObjectBodyTracking() {
    if (vaoID_ != 0) {
...
Simple3DObjectBodyTracking::Simple3DObjectBodyTracking(sl::Translation position, bool isStatic) : isStatic_(isStatic) {
    vaoID_ = 0;
...
void Simple3DObjectBodyTracking::addPt(sl::float3 pt) {
    vertices_.push_back(pt.x);
    vertices_.push_back(pt.y);
    vertices_.push_back(pt.z);
}

void Simple3DObjectBodyTracking::addClr(sl::float4 clr) {
...
void Simple3DObjectBodyTracking::addNormal(sl::float3 normal) {
    normals_.push_back(normal.x);
    normals_.push_back(normal.y);
    normals_.push_back(normal.z);
}

void Simple3DObjectBodyTracking::addPoints(std::vector<sl::float3> pts, sl::float4 base_clr) {
...
void Simple3DObjectBodyTracking::addPoint(sl::float3 pt, sl::float4 clr) {
    addPt(pt);
    addClr(clr);
...
void Simple3DObjectBodyTracking::addBoundingBox(std::vector<sl::float3> bbox, sl::float4 base_clr) {
...
void Simple3DObjectBodyTracking::addLine(sl::float3 p1, sl::float3 p2, sl::float3 clr) {
...
void Simple3DObjectBodyTracking::addCylinder(sl::float3 startPosition, sl::float3 endPosition, sl::float4 clr) {
...
void Simple3DObjectBodyTracking::addSphere(sl::float3 position, sl::float4 clr, float m_radius) {
...
void Simple3DObjectBodyTracking::pushToGPU() {
    if (!isStatic_ || vaoID_ == 0) {
        ...
        if (vertices_.size() > 0) {
            ...
            glBindBuffer(GL_ARRAY_BUFFER, vboID_[ShaderBodyTracking::ATTRIB_VERTICES_POS]);
            glBufferData(GL_ARRAY_BUFFER, vertices_.size() * sizeof (float), &vertices_[0], isStatic_ ? GL_STATIC_DRAW : GL_DYNAMIC_DRAW);
            glVertexAttribPointer(ShaderBodyTracking::ATTRIB_VERTICES_POS, 3, GL_FLOAT, GL_FALSE, 0, 0);
            glEnableVertexAttribArray(ShaderBodyTracking::ATTRIB_VERTICES_POS);
        }
        if (colors_.size() > 0) {
            glBindBuffer(GL_ARRAY_BUFFER, vboID_[ShaderBodyTracking::ATTRIB_COLOR_POS]);
            glBufferData(GL_ARRAY_BUFFER, colors_.size() * sizeof (float), &colors_[0], isStatic_ ? GL_STATIC_DRAW : GL_DYNAMIC_DRAW);
            glVertexAttribPointer(ShaderBodyTracking::ATTRIB_COLOR_POS, 4, GL_FLOAT, GL_FALSE, 0, 0);
            glEnableVertexAttribArray(ShaderBodyTracking::ATTRIB_COLOR_POS);
        }

        if (normals_.size() > 0) {
            glBindBuffer(GL_ARRAY_BUFFER, vboID_[ShaderBodyTracking::ATTRIB_NORMAL]);
            glBufferData(GL_ARRAY_BUFFER, normals_.size() * sizeof (float), &normals_[0], isStatic_ ? GL_STATIC_DRAW : GL_DYNAMIC_DRAW);
            glVertexAttribPointer(ShaderBodyTracking::ATTRIB_NORMAL, 3, GL_FLOAT, GL_FALSE, 0, 0);
            glEnableVertexAttribArray(ShaderBodyTracking::ATTRIB_NORMAL);
        }
...
void Simple3DObjectBodyTracking::clear() {
    vertices_.clear();
    colors_.clear();
    indices_.clear();
    normals_.clear();
...
void Simple3DObjectBodyTracking::setDrawingType(GLenum type) {
    drawingType_ = type;
}

void Simple3DObjectBodyTracking::draw() {
...
void Simple3DObjectBodyTracking::translate(const sl::Translation& t) {
    position_ = position_ + t;
}

void Simple3DObjectBodyTracking::setPosition(const sl::Translation& p) {
    position_ = p;
}

void Simple3DObjectBodyTracking::setRT(const sl::Transform& mRT) {
    position_ = mRT.getTranslation();
    rotation_ = mRT.getOrientation();
}

void Simple3DObjectBodyTracking::rotate(const sl::Orientation& rot) {
    rotation_ = rot * rotation_;
}

void Simple3DObjectBodyTracking::rotate(const sl::Rotation& m) {
    this->rotate(sl::Orientation(m));
}

void Simple3DObjectBodyTracking::setRotation(const sl::Orientation& rot) {
    rotation_ = rot;
}

void Simple3DObjectBodyTracking::setRotation(const sl::Rotation& m) {
    this->setRotation(sl::Orientation(m));
}

const sl::Translation& Simple3DObjectBodyTracking::getPosition() const {
    return position_;
}

sl::Transform Simple3DObjectBodyTracking::getModelMatrix() const {
    sl::Transform tmp;
...
ShaderBodyTracking::ShaderBodyTracking(const GLchar* vs, const GLchar* fs) {
    set(vs, fs);
}

void ShaderBodyTracking::set(const GLchar* vs, const GLchar* fs) {
...
ShaderBodyTracking::~ShaderBodyTracking() {
...
GLuint ShaderBodyTracking::getProgramId() {
    return programId_;
}

bool ShaderBodyTracking::compile(GLuint &shaderId, GLenum type, const GLchar* src) {
...
const sl::Translation CameraGLBodyTracking::ORIGINAL_FORWARD = sl::Translation(0, 0, 1);
const sl::Translation CameraGLBodyTracking::ORIGINAL_UP = sl::Translation(0, 1, 0);
const sl::Translation CameraGLBodyTracking::ORIGINAL_RIGHT = sl::Translation(1, 0, 0);

CameraGLBodyTracking::CameraGLBodyTracking(sl::Translation position, sl::Translation direction, sl::Translation vertical) {
...
CameraGLBodyTracking::~CameraGLBodyTracking() {
}

void CameraGLBodyTracking::update() {
...
void CameraGLBodyTracking::setProjection(float horizontalFOV, float verticalFOV, float znear, float zfar) {
...
const sl::Transform& CameraGLBodyTracking::getViewProjectionMatrix() const {
    return vpMatrix_;
}

float CameraGLBodyTracking::getHorizontalFOV() const {
    return horizontalFieldOfView_;
}

float CameraGLBodyTracking::getVerticalFOV() const {
    return verticalFieldOfView_;
}

void CameraGLBodyTracking::setOffsetFromPosition(const sl::Translation& o) {
    offset_ = o;
}

const sl::Translation& CameraGLBodyTracking::getOffsetFromPosition() const {
    return offset_;
}

void CameraGLBodyTracking::setDirection(const sl::Translation& direction, const sl::Translation& vertical) {
...
void CameraGLBodyTracking::translate(const sl::Translation& t) {
    position_ = position_ + t;
}

void CameraGLBodyTracking::setPosition(const sl::Translation& p) {
    position_ = p;
}

void CameraGLBodyTracking::rotate(const sl::Orientation& rot) {
    rotation_ = rot * rotation_;
    updateVectors();
}

void CameraGLBodyTracking::rotate(const sl::Rotation& m) {
    this->rotate(sl::Orientation(m));
}

void CameraGLBodyTracking::setRotation(const sl::Orientation& rot) {
    rotation_ = rot;
    updateVectors();
}

void CameraGLBodyTracking::setRotation(const sl::Rotation& m) {
    this->setRotation(sl::Orientation(m));
}

const sl::Translation& CameraGLBodyTracking::getPosition() const {
    return position_;
}

const sl::Translation& CameraGLBodyTracking::getForward() const {
    return forward_;
}

const sl::Translation& CameraGLBodyTracking::getRight() const {
    return right_;
}

const sl::Translation& CameraGLBodyTracking::getUp() const {
    return up_;
}

const sl::Translation& CameraGLBodyTracking::getVertical() const {
    return vertical_;
}

float CameraGLBodyTracking::getZNear() const {
    return znear_;
}

float CameraGLBodyTracking::getZFar() const {
    return zfar_;
}

void CameraGLBodyTracking::updateVectors() {
    forward_ = ORIGINAL_FORWARD * rotation_;
    up_ = ORIGINAL_UP * rotation_;
    right_ = sl::Translation(ORIGINAL_RIGHT * -1.f) * rotation_;
}

void CameraGLBodyTracking::updateView() {
    sl::Transform transformation(rotation_, (offset_ * rotation_) + position_);
    view_ = sl::Transform::inverse(transformation);
}

void CameraGLBodyTracking::updateVPMatrix() {
