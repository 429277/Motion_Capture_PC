...
class ShaderBodyTracking {
public:

    ShaderBodyTracking() : verterxId_(0), fragmentId_(0), programId_(0) {}
    ShaderBodyTracking(const GLchar* vs, const GLchar* fs);
    ~ShaderBodyTracking();

    // Delete the move constructor and move assignment operator
    ShaderBodyTracking(ShaderBodyTracking&&) = delete;
    ShaderBodyTracking& operator=(ShaderBodyTracking&&) = delete;

    // Delete the copy constructor and copy assignment operator
    ShaderBodyTracking(const ShaderBodyTracking&) = delete;
    ShaderBodyTracking& operator=(const ShaderBodyTracking&) = delete;
...
struct ShaderDataBodyTracking {
    ShaderBodyTracking it;
    GLuint MVP_Mat;
};

class Simple3DObjectBodyTracking {
public:

    Simple3DObjectBodyTracking();
    Simple3DObjectBodyTracking(sl::Translation position, bool isStatic);
    ~Simple3DObjectBodyTracking();
...
class CameraGLBodyTracking {
public:

    CameraGLBodyTracking() {
    }

    enum DIRECTION {
        UP, DOWN, LEFT, RIGHT, FORWARD, BACK
    };
    CameraGLBodyTracking(sl::Translation position, sl::Translation direction, sl::Translation vertical = sl::Translation(0, 1, 0)); // vertical = Eigen::Vector3f(0, 1, 0)
    ~CameraGLBodyTracking();
...
class GLViewerBodyTracking {
public:
    GLViewerBodyTracking();
    ~GLViewerBodyTracking();
    bool isAvailable();
...
private:
    ...
    ShaderDataBodyTracking shaderLine;
    ShaderDataBodyTracking shaderSK;

    sl::float3 bckgrnd_clr;

    CameraGLBodyTracking camera_;

    Simple3DObjectBodyTracking skeletons;
    Simple3DObjectBodyTracking floor_grid;
